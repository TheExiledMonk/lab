...
## LEDGER ENTRY 015 — DEV182 CURRENT-NATIVE PACKET / EXCITATION LAUNCH REPRESENTATION

- Date: 2026-08-12. DEV182 recovered the executable historical C25 and 100% launchers, audited the current DEV167/168/171 construction, and ran only small deterministic native replay controls. No historical propagation or observer output was reused as current physics.
- Historical reconciliation: the preserved PR16 code derives `EXPANDED_SIDE = round(sqrt(20000*64/18)) = 267`, hence **71,289** actual cell-centred ray launches. The `266×266 = 70,756` wording is an older nominal description. PR106 doubles the actual side to `534`, hence **285,156 = 4×71,289**. PR107's mismatch was exactly the consequence of reconstructing launch positions from a stale helper; actual launch-coordinate arrays are primary lineage.
- Current counts for each of the eight DEV171 realizations: `N_source_contact=49`, `N_independent_launch=1`, `N_packet=1`, `N_propagated_state=1`, and `N_receipt={12454,12579,12764,12801,12453,11594,11605,12288}`. The 49 cells are support/lineage cells of one finite packet field, and receipt events are positive bond-flux crossings—not launches or packets.
- Current packet semantics: DEV169/171 construct one finite native vector-state perturbation (fixed `+x` production direction, fixed `0.006` production amplitude, finite Gaussian-x × 7×7 node support) and add it to a loaded DEV167 state. It is a physical propagated state, not an old non-backreacting geometric ray. Exact fresh-state replays are deterministic and order-independent; simultaneous packets would instead define a different combined medium state.
- Closure: `OUTCOME_B`. Integer-node translations of an existing finite support are the only established position variation. Continuous packet coordinates, a direction continuum, production-free amplitude, a current launch surface, and packet-aware DEV168 receipt provenance remain underived. `LAUNCH_COUNT_DECOUPLED_FROM_SOURCE_LOADING=true` holds for exact-reset independent replays, but a true density run remains unauthorized pending a discrete launch-domain plus additive packet-lineage representation.
- Next frontier: `DISCRETE NATIVE LAUNCH-STATE DENSITY CONVERGENCE` only after that provenance/domain closure. Do not copy historical `266×266`, change source/medium resolution, interpolate a packet, change DEV167/168, or promote an observer result.

## LEDGER ENTRY 016 — DEV183 DISCRETE NATIVE LAUNCH DOMAIN + PACKET-AWARE RECEIPT LINEAGE

- Date: 2026-08-12. DEV183 froze the exact existing DEV169/171 packet (`+x`, amplitude `0.006`, finite native support) and enumerated translations without changing the packet, source loading, medium, DEV167 pair law, DEV168 physics, observer, or observational inputs.
- Domain: DEV167’s periodic N6 state makes transverse `y,z` index translations exact packet permutations. With the fixed `x=8` receipt face, `x` translations change propagation distance and are structurally admissible state permutations but **not** samples of the same launch experiment. The complete same-experiment launch domain is therefore `Z_11 × Z_11` at fixed `x`: two-dimensional, `N_admissible=121`, common to all DEV171 realizations.
- Provenance: `PacketAwareReceiptCollection/v1` preserves an unmodified `NativeReceivedState` plus aligned compact launch/packet/realization indices and JSON manifests. It rejects unknown/misaligned lineage and round-trips losslessly. The canonical reference launch reproduces frozen DEV177 realization-0 physical arrays byte-identically.
