#!/usr/bin/env python3
"""Launch-aware repair runner for PBUF full-state 100% observer coverage 001.

The original PR #106 lab correctly changes the launch from 25% to 100% coverage,
but the reused PR #104 helper `RET._binned_received_3d(...)` internally rebuilds
its initial 3D ray positions from the historical 25% launcher.  That hidden
assumption makes the 100% lane combine 285156 terminal rays with 71289 initial
rays and raises a NumPy broadcast error.

This repair changes no physics, propagation, decoder, weighting, target use, or
coverage rule.  It replaces only the observer-side receipt binning call with a
launch-aware equivalent that uses the exact x0/y0 arrays belonging to the lane
being decoded.  The original lab's main(), checks, aggregation, and output
format are otherwise preserved.
"""
...
def _binned_received_3d_with_launch(
    ...
    """PR #104 receipt extraction with the lane's actual launch coordinates."""
