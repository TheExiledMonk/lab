...
def main():
    ...
    # Publication is recorded after the content commit is actually accepted by
    # remote main.  Preserve that post-publication attestation on deterministic
    # replays rather than claiming it during generation.
    prior_contract = OUT / "final_contract.json"
    if prior_contract.exists():
        prior = json.loads(prior_contract.read_text())
        for key in ("COMMITTED", "PUSHED_DIRECTLY_TO_MAIN", "REMOTE_MAIN_VERIFIED", "WORKTREE_CLEAN"):
            if key in prior: flags[key] = prior[key]
